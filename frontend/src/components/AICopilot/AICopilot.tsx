...
import { MessageSquare, X, Send, Sparkles, FileText, Globe, Key, User, Play, Compass, Minimize2, Maximize2 } from "lucide-react";
...
export const AICopilot: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "ai",
      text: "Hello! I am your Helix AI Copilot. I understand your career goals, resume, and skills. How can I help you accelerate your growth today?",
  ...
  const handleActionClick = (action: typeof actions[0]) => {
    ...
    // Optionally redirect after initiating the context
    if (action.path) {
      ...
      <AnimatePresence>
        {!isOpen && (
          <motion.button
            ...
            className="w-14 h-14 rounded-full flex items-center justify-center bg-white/10 dark:bg-white/5 backdrop-blur-xl border border-white/20 dark:border-white/10 text-[var(--color-primary)] shadow-lg animate-pulse-glow transition-all duration-300"
            style={{ boxShadow: "0 0 16px 2px rgba(122, 23, 48, 0.2)" }}
            aria-label="Helix AI Copilot"
          >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`w-full max-w-[380px] sm:max-w-[420px] rounded-2xl border border-[var(--border)] bg-[var(--surface)]/95 shadow-2xl backdrop-blur-xl flex flex-col overflow-hidden transition-all duration-300 ${
